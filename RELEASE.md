...
2. Reinstall all dependencies `git clean -xfd && pnpm install`.
    * If there are any vulnerabilities fix them with `pnpm audit --fix`.
    * Make sure `pnpm-lock.yaml` is updated.
3. Check for all type and documentation errors by running `pnpm lint`.
...
7. Generate new documentation via `pnpm docs` and skip CSS changes.
...
2. Clean repository and publish new version: `git clean -xfd && pnpm publish`.
